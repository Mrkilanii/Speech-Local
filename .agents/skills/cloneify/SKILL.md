...
user pastes into a fresh Codex session to actually build the clone. Phases
...
- **Never** write to a path under `~/.Codex/skills/` or wherever this SKILL.md
...
hand you the single prompt to paste into a fresh Codex session to build it."*
Wait for go-ahead.
...
condition** — so the user can paste, set the finish line, and let Codex
...
> HOW TO RUN THIS (two moves in a fresh Codex session, in your target repo):
> 1. Paste everything below this line as your first message.
> 2. Then set the finish line so Codex keeps working until it's done:
...
Codex's `/goal` keeps working turn-after-turn until a small evaluator model
...
around something Codex's own output demonstrates. Derive it from the plan's
...
Keep it to one measurable end state + how Codex should prove it. `/goal` needs
Codex v2.1.139+, an accepted workspace-trust dialog, and hooks enabled; it
...
prompt into a fresh Codex session **opened in `<run-root>` (the target repo)**,
