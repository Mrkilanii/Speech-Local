---
name: icm-architect
description: Use when the user wants to design a folder structure for agent workflows, build a "workspace" for a repeating process, or restructure an existing folder/repo/vault into one that an AI agent can navigate reliably. Triggers on phrases like "ICM this," "structure this for agents," "build me a workspace for X," or requests to organize a messy project folder into something an agent (or a new collaborator) can orient in quickly.
---
...
- [assets/templates/](assets/templates/) — copyable starters: `AGENTS.md`, workspace `CONTEXT.md`, `stage-CONTEXT.md`, `node.md`, `schema.md`, `questionnaire.md`.
