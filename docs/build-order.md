...
| C2 | **Wiring and live use.** Third hotkey; "next line" as an indented paste; 0.5 s padding | **Done 15 Sep.** Grade from a mark dictated by Omar into Trace Table passed its tests after six live rounds (decision 10 addenda) |
| C3 | **Library names, scored.** 5,965 generated names; the 22-line pandas/numpy/matplotlib script in `docs/verification.md`, which the rules turn into the expected code under perfect recognition (`PythonScriptC3Tests`) | **Next.** Done when Omar dictates it and the differences from the expected code are counted and fixed where rules can |
